...
export class SidebarFilterComponent implements OnChanges {
  ...
  private setFieldOptionsOnSuccess(res: any): void {
    ...
    switch (this.field) {
      ...
      case 'customText1':
        this.options = [{ value: "Yes", label: `Remote` },           
          { value: "No", label: `On-Site`}, 
          { value: "Hybrid", label: `Travel Required`}
          ].filter((item: any) => {return item.value;});
        ...
        break;
      case 'employmentType':
        this.options = res.data.map((result: any) => {
          return {
            value: result.employmentType,
            label: `${result.employmentType}`,
          };
        }).filter((item: any) => {
          return item.value;
        });
