...
export class SearchService {
  ...
  public getRelatedJobs(filter:string, params:any = {}, count:number = 5):Observable<any>{
    let queryArray: string[] = [];
    params.query = `(isOpen:1) AND (isDeleted:0) AND ${filter}`;
    params.fields = this.fields;
    params.count = count;
    params.sort = SettingsService.settings.additionalJobCriteria.sort;
    params.showTotalMatched = true;
  ...
  public openJob(id: string | number): Observable<any> {
    return this.http.get(`${this.baseUrl}/query/JobBoardPost?where=(id=${id})&fields=${this.fields}`);
  }
